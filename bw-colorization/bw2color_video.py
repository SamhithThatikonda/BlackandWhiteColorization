...
arg.add_argument("-i", "--input", type=str,help="path to optional input video (webcam will be used otherwise)")
...
arg.add_argument("-w", "--width", type=int, default=500,help="input width dimension of frame")
args = vars(arg.parse_args())

# initialize a boolean to check the use of webcam or input video
web_cam = not args.get("input", False)
...
while True:
	...
	frame = vs.read()
	frame = frame if web_cam else frame[1]

	# if we are viewing a video and we did not grab a frame then we
	# have reached the end of the video
	if not web_cam and frame is None:
		break
